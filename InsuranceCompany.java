...
public class InsuranceCompany implements Cloneable {
    ...
    // creates and adds the User to users list if userID is unique, if not returns
    // false. Create a user object and reuse the addUser(User user) method
    ...
    // finds the user with the given userID by using findUser method and adds the
    // policy to the user,
    ...
    // Returns null if userID does not exist or policyID does not exist for the
    // given user
    ...
    // finds the user with the given userID (by calling findUser) and calls the
    // createThirdPartyPolicy for that user.
    // Returns false if the user does not exist or if User.createThirdPartyPolicy
    // returns false
    ...
    // finds the user with the given userID (by calling findUser) and calls the
    // createComprehensivePolicy for that user.
    // Returns false if the user does not exist or if User.createComprehensivePolicy
    // returns false
    ...
    // calls carPriceRiseAll method for the given user. Returns false if user cannot
    // be found
    ...
    // find the user by calling findUser and calls filterByCarModel for the given
    // user
    ...
    // find the user by calling findUser and calls filterByExpiryDate for the given
    // user
     ...
     * Iterate over a loop for all users and for each user call the filterByCarModel
     * method and
    ...
    // filters all the policies in the company by ExpiryDate across all users. The
    // same as above
    ...
    // prints all the users and for each user all the policies by calling
    // User.PrintPolicies(int flatRate)
    public void print() {
    ...
    public void reportPaymentPerCity(ArrayList<String> cities, ArrayList<Double> payments) // it is in the spec but not
                                                                                           // good
    {
    ...
    // for (User user:users)
    // {
    // count+=user.getTotalCountForCarModel(model); // by calling this method which
    // is not the same as spec
    // }
    ...
    public ArrayList<Integer> getTotalCountPerCarModel(ArrayList<String> carModels) {
        ...
        for (User user : users) {
            ...
            for (int i = 0; i < userCounts.size(); i++) {
                totalCounts.set(i, totalCounts.get(i) + userCounts.get(i));// for each element of total add the user
                                                                           // count
            }
        }
        return totalCounts;
    ...
    public void reportPaymentsPerCarModel(ArrayList<String> carModels, ArrayList<Integer> counts,
        ...
        for (int i = 0; i < counts.size(); i++)
            System.out.println(carModels.get(i) + " \t \t \t " + payments.get(i) + " \t \t \t "
                    + payments.get(i) / (double) counts.get(i));
    }
    ...
    public ArrayList<User> sortUsers() throws CloneNotSupportedException {
        ArrayList<User> shallowCopy = shallowCopyUsers();
